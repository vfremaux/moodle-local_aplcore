    ...
    var init_course_selector = function(name, hash, extrafields, lastsearch) {
        ...
        var course_selector = {
            ...
            send_query: function(forceresearch) {
                ...
                var xhr = async() => {
                    const response = await searchForCourses(this.name, value, this.get_option('searchanywhere') ? 1 : 0);
                    try {
             ...
             */
            output_options: async function(data) {
                ...
                for (var key in data.results) {
                    let groupdata = data.results[key];
                ...
                if (!count) {
                    let searchstr;
                    if (this.lastsearch !== '') {
                        const str = await corestr.get_string('nomatchingcourses', 'local_aplcore');
                ...
                if (this.get_option('preserveselected') && selectedcourses) {
                    let searchstr;
                    const str = await corestr.get_string('previouslyselectedcourses', 'local_aplcore');
                    searchstr = this.insert_search_into_str(str, this.lastsearch);
