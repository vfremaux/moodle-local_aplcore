...
$component = required_param('plugin', PARAM_TEXT);
$promanagerclass = '\\'.$component.'\\pro_manager';
$promanager = $promanagerclass::instance();
$returnurl = $promanager->return_url();

$componentpath = $promanager::$componentpath;

require_once($CFG->dirroot.'/local_aplcore/pro/forms/form_getkey.php');

$url = new moodle_url('/local_aplcore/pro/getpluginkey.php', ['plugin' => $component]);
$PAGE->set_url($url);
...
$mform = new GetKey_Form($url, ['manager' => $promanager, 'options' => $optionsmenu, 'plugin' => $component]);
...
if ('confirm' == optional_param('what', '', PARAM_ALPHA)) {
    ...
    if (empty($data->provider) || empty($data->partnerkey) || empty($data->activationoption)) {
        echo $OUTPUT->header();
        echo $OUTPUT->notification(get_string('missingparams', 'local_aplcore'), 'error');
