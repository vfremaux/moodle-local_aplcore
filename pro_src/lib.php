...
class license_manager {
    ...
    /**
     * When giving a feature, checks if the feature is part of pro set
     * @param string $feature the feature to check 
     * @return boolean true if feature is only on "pro" set.
     */
    public function is_pro($feature) {
        if (empty($feature)) {
            throw new coding_exception("Feature must be a non empty string");
        }
        $func = static::$shortcomponent.'_supports_feature';
        $supported = $func(null, true);
        $prothings = $supported['pro'];
        [$feat, $subfeat] = explode('/', $feature);
        if (!array_key_exists($feat, $prothings)) {
            return false;
        }
        if (!array_key_exists($subfeat, $prothings[$feat])) {
            return false;
        }
        return true;
    }
    ...
    public function add_settings(&$admin, &$settings) {
        ...
        // Now pro is uniquely provided by local_aplcore. Call once per component to check.
        // No more calls to local amd "pro" JS.
        $PAGE->requires->js_call_amd('local_aplcore/pro_plugins', 'init', [
            static::$component, // Component.
            static::$shortcomponent, // Shortcomponent.
            static::$componentpath, // Componentpath.
        ]);
        ...
        $versioninfo = $this->get_version_info('distribution');
        if ($versioninfo === 'marketplace') {
            $desc .= '<br>';
            $e = new StdClass();
            $e->lang = $CFG->lang;
            $e->plugin = static::$component;
            $e->wwwroot = $CFG->wwwroot;
            $desc .= new lang_string('marketplaceonboarding', 'local_aplcore', $e);
        }
    ...
    /**
     * Retreives some version additional metadata.
     */
    public function get_version_info($key = null) {
        $componentpath = \core_component::get_component_directory(static::$component);
        if (is_null($componentpath)) {
            return null;
        }
        /*
         * This is an obsfuscator turnaround, in order to get an
         * effective local variable clearnamed $plugin.
         */
        $varname = 'plugin';
        $$varname = new StdClass();
        include($componentpath.'/version.php');
